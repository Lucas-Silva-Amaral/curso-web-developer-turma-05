...
const order = (operation) => {
  ...
  const salCre = (users, show) => {
    for (let i = 0; i < users.length; i++) {
      for (let j = 0; j < users.length; j++) {
        if (users[i].salario < users[j].salario) {
          const temp = users[i]
          users[i] = users[j]
          users[j] = temp
        }
  ...
  const senCre = (users, show) => {
    ...
    for (let i = 0; i < users.length; i++) {
      for (let j = 0; j < users.length; j++) {
        if (level[users[i].senioridade] < level[users[j].senioridade]) {
          temp = users[i]
          users[i] = users[j]
          users[j] = temp
        }
  ...
  switch (operation) {
    case "AgeCre":
      return ageCre(users, show)
    case "AgeDecre":
      return ageDecre(users, show)
    case "salCre":
      return ageDecre(users, show)
    case "salDecre":
      return salDecre(users, show)
    case "senCre":
      return senCre(users, show)
    case "senDecre":
      return senDecre(users, show)
    default:
      return "invalid"
      break
  }
